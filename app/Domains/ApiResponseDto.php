...
class ApiResponseDto implements ArrayAccess, Arrayable, Jsonable
{
    /**
     * @var bool 是否正常处理请求
     */
    ...
    public function success(array $data = null, bool $ignoreErrorCode = true)
    {
        $this->success = true;
        if (!empty($data)) {
            $this->data = $data;
        ...
        if (!$ignoreErrorCode) {
            ...
            $this->errorMessage = self::ERROR['00000'];
        }

        return $this;
    }

    public function error(string $errorCode, array $data = null)
    {
        if (!key_exists($errorCode, self::ERROR)) {
            throw new InvalidArgumentException('未定义的错误');
        }
        $this->success = false;
        $this->errorCode = $errorCode;
        $this->errorMessage = self::ERROR[$errorCode];
        if (!empty($data)) {
            $this->data = $data;
        }
